...
<?php
                    $id = $_GET['id'];
                    $data = mysqli_query($konek, "select  * from pembayaran where id_pembayaran='$id'");
...
<section class="section contact-form">
	<div class="container">
        ...
        <div class="form-group">
                                <input type="text" class="form-control" hidden name="id_pembayaran" autocomplete="off" value="<?php echo $tampil['id_pembayaran']; ?>">
                            </div>
			<div class="col-md-6 mt-5">
                <label class="form-label fw-bold">Nama pembayaran</label>
				<input type="text" class="form-control main" name="pembayaran" id="nama" placeholder="Nama pembayaran" value="<?php echo $tampil['pembayaran']; ?>">
			</div>
            <div class="mb-3">
  <label for="exampleFormControlTextarea1" class="form-label">Nomor pembayaran</label>
  <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="nomor"><?php echo $tampil['nomor']; ?></textarea>
</div>
<div class="mb-3">
  <label for="formFileSm" class="form-label">Small file input example</label>
  <input class="form-control form-control-sm" type="file" id="gambar" name="nama_file" value="<?php echo $tampil['gambar'];?>">
</div>
<img src="<?php echo $tampil['gambar']; ?>" style="width: 20%; height:20%;">
                                <p>file harus Jpg,Jpeg,PNG</p>
<?php
                    }
            ?>
			<div class="col-12 text-center mt-4">
				<button class="btn btn-primary btn-lg btn-block" type="submit" style="margin-top:20px;" name="btnedit">Update Pembayaran</button>
    ...
    if (!empty($_FILES["nama_file"]["tmp_name"])) {
        ...
        if ($jenis_gambar == "image/jpeg" || $jenis_gambar == "image/jpg" || $jenis_gambar == "image/gif" || $jenis_gambar == "image/png") {
            ...
            if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $gambar)) {
                $result = mysqli_query(
                    $konek,
                    "UPDATE pembayaran SET pembayaran='$pembayaran',nomor='$nomor',gambar='$gambar'WHERE id_pembayaran = '$id_pembayaran'"
                );
