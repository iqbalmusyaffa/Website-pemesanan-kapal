...
<?php
                    $id = $_GET['id'];
                    $data = mysqli_query($konek, "select  * from tujuan where id_tujuan='$id'");
		...
		<div class="row">
			<div class="col-12">
				<div class="section-title">
					<h3>Update <span class="alternate">Data Tujuan</span></h3>
				</div>
			</div>
		</div>
		<form action="" method="POST" class="row" enctype="multipart/form-data">
        <div class="form-group">
                                <input type="text" class="form-control" hidden name="id_user" autocomplete="off" value="<?php echo $tampil['id_tujuan']; ?>">
                            </div>
			<div class="col-md-6 mt-5">
                <label class="form-label fw-bold">Nama tujuan</label>
				<input type="text" class="form-control main" name="tujuan" id="nama" placeholder="Nama Tujuan" value="<?php echo $tampil['tujuan']; ?>">
			</div>
            <div class="mb-3">
  <label for="exampleFormControlTextarea1" class="form-label">Deskripsi</label>
  <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="dekripsi"><?php echo $tampil['dekripsi']; ?></textarea>
</div>
<div class="mb-3">
...
<option selected>Pilih Kategori dibawah ini </option>
  ...
  $query    =mysqli_query($konek, "SELECT * FROM kategori");
			...
			<div class="col-12 text-center mt-4">
				<button class="btn btn-primary btn-lg btn-block" type="submit" style="margin-top:20px;" name="btnedit">Update Tujuan</button>
    ...
    if (!empty($_FILES["nama_file"]["tmp_name"])) {
        ...
        if ($jenis_gambar == "image/jpeg" || $jenis_gambar == "image/jpg" || $jenis_gambar == "image/gif" || $jenis_gambar == "image/png") {
            ...
            if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $foto)) {
                $result = mysqli_query(
                    $konek,
                    "UPDATE tujuan SET tujuan='$tujuan',dekripsi='$dekripsi',kategori_id='$kategori_id',foto='$foto'WHERE id_tujuan='$id_tujuan'"
                );
